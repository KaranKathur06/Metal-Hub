...
CREATE POLICY "taxonomy_admin_write" ON public.taxonomy
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin', 'marketing'))
  );
...
CREATE POLICY "rfqs_own_read" ON public.rfqs
  FOR SELECT USING (buyer_profile_id = auth.uid() OR EXISTS (
    SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin', 'moderator', 'seller')
  ));
